// Kalura Service Agreement module v1.7.2-readonly-7col
...
interface SupportScheduleRow {
  supportItemName: string;
  supportItemNumber: string;
  supportItemDescription: string;
  serviceDayAndTime: string;
  frequencyServiceArrangement: string;
  location: string;
  pricePerHourOrUnit: string;
  quantity: string;
  totalPrice: string;
}
...
export default function ServiceAgreement({
  ...
  supportItem4: supportItem4Prop,
  supportDescription4: supportDescription4Prop,
  supportPrice4: supportPrice4Prop,
  supportDelivery4: supportDelivery4Prop,
  supportItem5: supportItem5Prop,
  supportDescription5: supportDescription5Prop,
  supportPrice5: supportPrice5Prop,
  supportDelivery5: supportDelivery5Prop,
  supportItem6: supportItem6Prop,
  supportDescription6: supportDescription6Prop,
  supportPrice6: supportPrice6Prop,
  supportDelivery6: supportDelivery6Prop,
  ...
  const [supportItem4, setSupportItem4] = useState(supportItem4Prop || "");
  const [supportDescription4, setSupportDescription4] = useState(supportDescription4Prop || "");
  const [supportPrice4, setSupportPrice4] = useState(supportPrice4Prop || "");
  const [supportDelivery4, setSupportDelivery4] = useState(supportDelivery4Prop || "");

  const [supportItem5, setSupportItem5] = useState(supportItem5Prop || "");
  const [supportDescription5, setSupportDescription5] = useState(supportDescription5Prop || "");
  const [supportPrice5, setSupportPrice5] = useState(supportPrice5Prop || "");
  const [supportDelivery5, setSupportDelivery5] = useState(supportDelivery5Prop || "");

  const [supportItem6, setSupportItem6] = useState(supportItem6Prop || "");
  const [supportDescription6, setSupportDescription6] = useState(supportDescription6Prop || "");
  const [supportPrice6, setSupportPrice6] = useState(supportPrice6Prop || "");
  const [supportDelivery6, setSupportDelivery6] = useState(supportDelivery6Prop || "");

  // The Schedule of Supports uses the exact 7-column layout from the supplied
  // reference: 6 editable rows, with Item Name + Number and Service Day/Time
  // + Location combined into their respective cells.
  const emptySupportScheduleRow = (): SupportScheduleRow => ({
    supportItemName: "",
    supportItemNumber: "",
    supportItemDescription: "",
    serviceDayAndTime: "",
    frequencyServiceArrangement: "",
    location: "",
    pricePerHourOrUnit: "",
    quantity: "",
    totalPrice: "",
  ...
  const [supportScheduleRows, setSupportScheduleRows] = useState<SupportScheduleRow[]>([
    { ...emptySupportScheduleRow(), supportItemName: supportItem1Prop || "", supportItemDescription: supportDescription1Prop || "", pricePerHourOrUnit: supportPrice1Prop || "", frequencyServiceArrangement: supportDelivery1Prop || "" },
    { ...emptySupportScheduleRow(), supportItemName: supportItem2Prop || "", supportItemDescription: supportDescription2Prop || "", pricePerHourOrUnit: supportPrice2Prop || "", frequencyServiceArrangement: supportDelivery2Prop || "" },
    { ...emptySupportScheduleRow(), supportItemName: supportItem3Prop || "", supportItemDescription: supportDescription3Prop || "", pricePerHourOrUnit: supportPrice3Prop || "", frequencyServiceArrangement: supportDelivery3Prop || "" },
    { ...emptySupportScheduleRow(), supportItemName: supportItem4Prop || "", supportItemDescription: supportDescription4Prop || "", pricePerHourOrUnit: supportPrice4Prop || "", frequencyServiceArrangement: supportDelivery4Prop || "" },
    { ...emptySupportScheduleRow(), supportItemName: supportItem5Prop || "", supportItemDescription: supportDescription5Prop || "", pricePerHourOrUnit: supportPrice5Prop || "", frequencyServiceArrangement: supportDelivery5Prop || "" },
    { ...emptySupportScheduleRow(), supportItemName: supportItem6Prop || "", supportItemDescription: supportDescription6Prop || "", pricePerHourOrUnit: supportPrice6Prop || "", frequencyServiceArrangement: supportDelivery6Prop || "" },
  ]);

  const updateSupportScheduleCell = (
    rowIndex: number,
    field: keyof SupportScheduleRow,
    value: string,
  ) => {
    setSupportScheduleRows((rows) =>
      rows.map((row, index) =>
  ...
  useEffect(() => {
    ...
    async function loadAgreement() {
      try {
        ...
        setSupportItem4(getSavedValue(data, "supportItem4", supportItem4Prop || ""));
        setSupportDescription4(getSavedValue(data, "supportDescription4", supportDescription4Prop || ""));
        setSupportPrice4(getSavedValue(data, "supportPrice4", supportPrice4Prop || ""));
        setSupportDelivery4(getSavedValue(data, "supportDelivery4", supportDelivery4Prop || ""));
        setSupportItem5(getSavedValue(data, "supportItem5", supportItem5Prop || ""));
        setSupportDescription5(getSavedValue(data, "supportDescription5", supportDescription5Prop || ""));
        setSupportPrice5(getSavedValue(data, "supportPrice5", supportPrice5Prop || ""));
        setSupportDelivery5(getSavedValue(data, "supportDelivery5", supportDelivery5Prop || ""));
        setSupportItem6(getSavedValue(data, "supportItem6", supportItem6Prop || ""));
        setSupportDescription6(getSavedValue(data, "supportDescription6", supportDescription6Prop || ""));
        setSupportPrice6(getSavedValue(data, "supportPrice6", supportPrice6Prop || ""));
        setSupportDelivery6(getSavedValue(data, "supportDelivery6", supportDelivery6Prop || ""));

        const savedSupportRows = getSavedValue<unknown>(data, "supportScheduleRows", null);
        if (Array.isArray(savedSupportRows)) {
          const rows = savedSupportRows.slice(0, 6).map((row) => {
            const value = row && typeof row === "object"
              ? row as Record<string, unknown>
              : {};
            return {
              supportItemName: String(value.supportItemName ?? ""),
              supportItemNumber: String(value.supportItemNumber ?? ""),
              supportItemDescription: String(value.supportItemDescription ?? ""),
              serviceDayAndTime: String(value.serviceDayAndTime ?? ""),
              frequencyServiceArrangement: String(value.frequencyServiceArrangement ?? ""),
              location: String(value.location ?? ""),
              pricePerHourOrUnit: String(value.pricePerHourOrUnit ?? ""),
              quantity: String(value.quantity ?? ""),
              totalPrice: String(value.totalPrice ?? ""),
            };
          });
          while (rows.length < 6) rows.push(emptySupportScheduleRow());
          setSupportScheduleRows(rows);
        }
  ...
  useEffect(() => {
    ...
    const agreementData = {
      ...
      supportItem4, supportDescription4, supportPrice4, supportDelivery4,
      supportItem5, supportDescription5, supportPrice5, supportDelivery5,
      supportItem6, supportDescription6, supportPrice6, supportDelivery6,
      supportScheduleRows,
      // Legacy Section 24 values are retained for backward-compatible storage.
      consentPersonalInfo, consentProviders, consentResearch, consentNdisPortal,
      consentNdisPlan, consentNdisPortion, photoService, photoMarketing,
      photoPublications, newsletter, serviceOpportunities,
      consentgiven: consentGiven,
      consentname: consentName,
      understanding, communicationUsed, communicationDeclined,
      communicationNotRequired, independentAdvice, participantSigningName,
      participantSigningDate, representativeName, representativeDate,
      kaluraSigningName, kaluraSignature, kaluraDate,
    };

    saveTimer.current = setTimeout(async () => {
      try {
        setSyncStatus("saving");
        setSyncMessage("Saving editor changes...");
        ...
        setSyncStatus("failed");
        setSyncMessage("Unable to save editor changes.");
      }
    }, 600);
  ...
  return (
    <div className={styles.document}>
      ...
      <fieldset
        ...
        aria-label="Editor-only Service Agreement sections"
      >
      ...
      <div className={styles.majorSection}>
        ...
        >
          ...
          <thead>
            <tr>
              {[
                ...
                "Service Day, Time and Location",
                "Frequency/Service Arrangement",
                "Price per Hour",
                "Quantity",
                "Total Price",
              ].map((headingText) => (
          ...
          <tbody>
            {supportScheduleRows.map((row, rowIndex) => (
              <tr key={rowIndex} style={{ height: "98px" }}>
                <td style={{ padding: "4px", verticalAlign: "top" }}>
                  <TextInput
                    value={row.supportItemName}
                    onChange={(value) => updateSupportScheduleCell(rowIndex, "supportItemName", value)}
                  />
                  <TextInput
                    value={row.supportItemNumber}
                    onChange={(value) => updateSupportScheduleCell(rowIndex, "supportItemNumber", value)}
                  />
                </td>
                <td style={{ padding: "4px", verticalAlign: "top" }}>
                  <TextArea
                    value={row.supportItemDescription}
                    onChange={(value) => updateSupportScheduleCell(rowIndex, "supportItemDescription", value)}
                    rows={4}
                  />
                </td>
                <td style={{ padding: "4px", verticalAlign: "top" }}>
                  <TextArea
                    value={row.serviceDayAndTime}
                    onChange={(value) => updateSupportScheduleCell(rowIndex, "serviceDayAndTime", value)}
                    rows={2}
                  />
                  <TextArea
                    value={row.location}
                    onChange={(value) => updateSupportScheduleCell(rowIndex, "location", value)}
                    rows={2}
                  />
                </td>
                <td style={{ padding: "4px", verticalAlign: "top" }}>
                  <TextArea
                    value={row.frequencyServiceArrangement}
                    onChange={(value) => updateSupportScheduleCell(rowIndex, "frequencyServiceArrangement", value)}
                    rows={4}
                  />
                </td>
                <td style={{ padding: "4px", verticalAlign: "top" }}>
                  <TextInput
                    value={row.pricePerHourOrUnit}
                    onChange={(value) => updateSupportScheduleCell(rowIndex, "pricePerHourOrUnit", value)}
                  />
                </td>
                <td style={{ padding: "4px", verticalAlign: "top" }}>
                  <TextInput
                    value={row.quantity}
                    onChange={(value) => updateSupportScheduleCell(rowIndex, "quantity", value)}
                  />
                </td>
                <td style={{ padding: "4px", verticalAlign: "top" }}>
                  <TextInput
                    value={row.totalPrice}
                    onChange={(value) => updateSupportScheduleCell(rowIndex, "totalPrice", value)}
                  />
                </td>
              </tr>
            ))}
      ...
      {/* Section 24 — Participant Consent Form intentionally disabled. */}

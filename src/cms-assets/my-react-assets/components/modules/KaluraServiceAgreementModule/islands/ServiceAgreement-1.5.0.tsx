...
export default function ServiceAgreement({
  ...
  const [ndisPlanAttached, setNdisPlanAttached] = useState(false);
  const [ndisPlanNotShared, setNdisPlanNotShared] = useState(false);

  const [claimNonFace, setClaimNonFace] = useState<"yes" | "no" | "">("");
  ...
  const [claimReports, setClaimReports] = useState<"yes" | "no" | "">("");

  const [conditions, setConditions] = useState("");
  const [conditionReasons, setConditionReasons] = useState("");

  const [fundingComponent, setFundingComponent] = useState("");
  const [fundingDates, setFundingDates] = useState("");
  const [fundingNotes, setFundingNotes] = useState("");

  const [supportItem1, setSupportItem1] = useState("");
  const [supportDescription1, setSupportDescription1] = useState("");
  const [supportPrice1, setSupportPrice1] = useState("");
  const [supportDelivery1, setSupportDelivery1] = useState("");

  const [supportItem2, setSupportItem2] = useState("");
  const [supportDescription2, setSupportDescription2] = useState("");
  const [supportPrice2, setSupportPrice2] = useState("");
  const [supportDelivery2, setSupportDelivery2] = useState("");

  const [supportItem3, setSupportItem3] = useState("");
  const [supportDescription3, setSupportDescription3] = useState("");
  const [supportPrice3, setSupportPrice3] = useState("");
  const [supportDelivery3, setSupportDelivery3] = useState("");
  ...
  const [communicationUsed, setCommunicationUsed] = useState(false);
  const [communicationDeclined, setCommunicationDeclined] = useState(false);
  const [communicationNotRequired, setCommunicationNotRequired] =
    useState(false);
  const [independentAdvice, setIndependentAdvice] = useState(false);

  const [participantSigningName, setParticipantSigningName] = useState("");
  const [participantSigningDate, setParticipantSigningDate] = useState("");

  const [representativeName, setRepresentativeName] = useState("");
  const [representativeDate, setRepresentativeDate] = useState("");

  const [kaluraSigningName, setKaluraSigningName] = useState("");
  const [kaluraSignature, setKaluraSignature] = useState("");
  const [kaluraDate, setKaluraDate] = useState("");
  ...
  useEffect(() => {
    if (!quoteId || isQuoteBlueprint || isInPreviewer) {
      return;
    }
    ...
    async function loadAgreement() {
      try {
        ...
        setFullName(
          getSavedValue(data, "participantFullName", participantFullName || "")
        );
        setDateOfBirth(
          getSavedValue(
            data,
        ...
        setAddress(
          getSavedValue(data, "participantAddress", participantAddress || "")
        );
        setSuburb(
          getSavedValue(data, "participantSuburb", participantSuburb || "")
        );
        setState(
          getSavedValue(data, "participantState", participantState || "")
        );
        setPostcode(
          getSavedValue(data, "participantPostcode", participantPostcode || "")
        );

        setNdisNumberValue(
          getSavedValue(data, "ndisNumber", ndisNumber || "")
        );
        setNdisStartDate(
          getSavedValue(
            data,
        ...
        setNdisEndDate(
          getSavedValue(data, "ndisPlanEndDate", ndisPlanEndDate || "")
        );

        setStartDate(
          getSavedValue(data, "agreementStartDate", agreementStartDate || "")
        );
        setEndDate(
          getSavedValue(data, "agreementEndDate", agreementEndDate || "")
        );

        setPlanManagement(getSavedValue(data, "planManagement", ""));

        setNdisPlanAttached(
          Boolean(getSavedValue(data, "ndisPlanAttached", false))
        );
        setNdisPlanNotShared(
          Boolean(getSavedValue(data, "ndisPlanNotShared", false))
        );

        setSupportItem1(getSavedValue(data, "supportItem1", ""));
        setSupportDescription1(
          getSavedValue(data, "supportDescription1", "")
        );
        setSupportPrice1(getSavedValue(data, "supportPrice1", ""));
        setSupportDelivery1(getSavedValue(data, "supportDelivery1", ""));

        setSupportItem2(getSavedValue(data, "supportItem2", ""));
        setSupportDescription2(
          getSavedValue(data, "supportDescription2", "")
        );
        setSupportPrice2(getSavedValue(data, "supportPrice2", ""));
        setSupportDelivery2(getSavedValue(data, "supportDelivery2", ""));

        setSupportItem3(getSavedValue(data, "supportItem3", ""));
        setSupportDescription3(
          getSavedValue(data, "supportDescription3", "")
        );
        setSupportPrice3(getSavedValue(data, "supportPrice3", ""));
        setSupportDelivery3(getSavedValue(data, "supportDelivery3", ""));

        setClaimNonFace(getSavedValue(data, "claimNonFace", ""));
        setClaimTravelLabour(getSavedValue(data, "claimTravelLabour", ""));
        setClaimTravelNonLabour(
          getSavedValue(data, "claimTravelNonLabour", "")
        );
        setClaimCancellation(getSavedValue(data, "claimCancellation", ""));
        setClaimReports(getSavedValue(data, "claimReports", ""));

        setConditions(getSavedValue(data, "conditions", ""));
        setConditionReasons(getSavedValue(data, "conditionReasons", ""));

        setFundingComponent(getSavedValue(data, "fundingComponent", ""));
        setFundingDates(getSavedValue(data, "fundingDates", ""));
        setFundingNotes(getSavedValue(data, "fundingNotes", ""));

        setConsentPersonalInfo(
          getSavedValue(data, "consentPersonalInfo", "")
        );
        ...
        setConsentNdisPortion(
          getSavedValue(data, "consentNdisPortion", "")
        );

        setPhotoService(getSavedValue(data, "photoService", ""));
        setPhotoMarketing(getSavedValue(data, "photoMarketing", ""));
        setPhotoPublications(
          getSavedValue(data, "photoPublications", "")
        );

        setNewsletter(getSavedValue(data, "newsletter", ""));
        setServiceOpportunities(
          getSavedValue(data, "serviceOpportunities", "")
        );

        setUnderstanding(Boolean(getSavedValue(data, "understanding", false)));
        setCommunicationUsed(
          Boolean(getSavedValue(data, "communicationUsed", false))
        );
        setCommunicationDeclined(
          Boolean(getSavedValue(data, "communicationDeclined", false))
        );
        setCommunicationNotRequired(
          Boolean(getSavedValue(data, "communicationNotRequired", false))
        );
        setIndependentAdvice(
          Boolean(getSavedValue(data, "independentAdvice", false))
        );

        setParticipantSigningName(
          getSavedValue(data, "participantSigningName", "")
        );
        setParticipantSigningDate(
          getSavedValue(data, "participantSigningDate", "")
        );

        setRepresentativeName(
          getSavedValue(data, "representativeName", "")
        );
        setRepresentativeDate(
          getSavedValue(data, "representativeDate", "")
        );

        setKaluraDate(getSavedValue(data, "kaluraDate", ""));

        // The state setters above restore the saved agreement. Do not treat
        // that restoration as a participant edit and immediately POST it back.
        ...
        // Do not enable saving when the existing record could not be loaded.
        hasLoadedAgreement.current = false;
        setSyncStatus("failed");
        setSyncMessage("Unable to load the saved Service Agreement.");
      }
    }
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(async () => {
      try {
        ...
        setSyncMessage(
          "Your changes were saved. Updating the signing document..."
        );
        ...
        const pollStatus = async () => {
          try {
            ...
            setSyncStatus("updating");
            setSyncMessage(
              message || "Updating the signing document..."
            );
        ...
        statusTimer.current = setTimeout(pollStatus, 1000);
      } catch (error) {

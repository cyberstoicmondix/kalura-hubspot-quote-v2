// Kalura Service Agreement module v1.5.0
...
export default function ServiceAgreement({
  ...
  const [consentPersonalInfo, setConsentPersonalInfo] = useState<
    "yes" | "no" | ""
  >("");
  ...
  // HubSpot owns editor-controlled Sections 1–23 and 25. We therefore do
  // not call the WordPress agreement endpoint while the quote is being edited.
  // React state is kept in sync with the current HubSpot module field values.
  useEffect(() => {
    if (!isInEditor) {
      return;
    }

    setFullName(participantFullName || "");
    setDateOfBirth(participantDateOfBirth || "");
    setAddress(participantAddress || "");
    setSuburb(participantSuburb || "");
    setState(participantState || "");
    setPostcode(participantPostcode || "");
    setNdisNumberValue(ndisNumber || "");
    setNdisStartDate(ndisPlanStartDate || "");
    setNdisEndDate(ndisPlanEndDate || "");
    setStartDate(agreementStartDate || "");
    setEndDate(agreementEndDate || "");
    setPlanManagement(planManagementProp || "");
    setNdisPlanAttached(Boolean(ndisPlanAttachedProp));
    setNdisPlanNotShared(Boolean(ndisPlanNotSharedProp));
    setClaimNonFace(claimNonFaceProp || "");
    setClaimTravelLabour(claimTravelLabourProp || "");
    setClaimTravelNonLabour(claimTravelNonLabourProp || "");
    setClaimCancellation(claimCancellationProp || "");
    setClaimReports(claimReportsProp || "");
    setConditions(conditionsProp || "");
    setConditionReasons(conditionReasonsProp || "");
    setFundingComponent(fundingComponentProp || "");
    setFundingDates(fundingDatesProp || "");
    setFundingNotes(fundingNotesProp || "");
    setSupportItem1(supportItem1Prop || "");
    setSupportDescription1(supportDescription1Prop || "");
    setSupportPrice1(supportPrice1Prop || "");
    setSupportDelivery1(supportDelivery1Prop || "");
    setSupportItem2(supportItem2Prop || "");
    setSupportDescription2(supportDescription2Prop || "");
    setSupportPrice2(supportPrice2Prop || "");
    setSupportDelivery2(supportDelivery2Prop || "");
    setSupportItem3(supportItem3Prop || "");
    setSupportDescription3(supportDescription3Prop || "");
    setSupportPrice3(supportPrice3Prop || "");
    setSupportDelivery3(supportDelivery3Prop || "");
    setUnderstanding(Boolean(understandingProp));
    setCommunicationUsed(Boolean(communicationUsedProp));
    setCommunicationDeclined(Boolean(communicationDeclinedProp));
    setCommunicationNotRequired(Boolean(communicationNotRequiredProp));
    setIndependentAdvice(Boolean(independentAdviceProp));
    setParticipantSigningName(participantSigningNameProp || "");
    setParticipantSigningDate(participantSigningDateProp || "");
    setRepresentativeName(representativeNameProp || "");
    setRepresentativeDate(representativeDateProp || "");
    setKaluraSigningName(kaluraSigningNameProp || "");
    setKaluraSignature(kaluraSignatureProp || "");
    setKaluraDate(kaluraDateProp || "");
  }, [
    isInEditor,
    participantFullName,
    participantDateOfBirth,
    ...
    agreementEndDate,
    planManagementProp,
    ndisPlanAttachedProp,
    ndisPlanNotSharedProp,
    claimNonFaceProp,
    claimTravelLabourProp,
    claimTravelNonLabourProp,
    claimCancellationProp,
    claimReportsProp,
    conditionsProp,
    conditionReasonsProp,
    fundingComponentProp,
    fundingDatesProp,
    fundingNotesProp,
    supportItem1Prop,
    supportDescription1Prop,
    supportPrice1Prop,
    supportDelivery1Prop,
    supportItem2Prop,
    supportDescription2Prop,
    supportPrice2Prop,
    supportDelivery2Prop,
    supportItem3Prop,
    supportDescription3Prop,
    supportPrice3Prop,
    supportDelivery3Prop,
    understandingProp,
    communicationUsedProp,
    communicationDeclinedProp,
    communicationNotRequiredProp,
    independentAdviceProp,
    participantSigningNameProp,
    participantSigningDateProp,
    representativeNameProp,
    representativeDateProp,
    kaluraSigningNameProp,
    kaluraSignatureProp,
    kaluraDateProp,
  ]);

  // Editor mode reads/writes the rendered agreement fields directly to the
  // dedicated editor endpoint. HubSpot module props provide the initial values;
  // WordPress persists inline edits so they survive reloads and publishing.
  useEffect(() => {
    if (!quoteId || isQuoteBlueprint || isInPreviewer || !isInEditor) {
      return;
    }

    let cancelled = false;

    async function loadEditorAgreement() {
      try {
        const response = await fetch(`${API_BASE}/${quoteId}/editor`, {
          cache: "no-store",
        });
        ...
        if (hasSavedData) {
          setFullName(getSavedValue(data, "participantFullName", fullName));
          setDateOfBirth(getSavedValue(data, "participantDateOfBirth", dateOfBirth));
          setAddress(getSavedValue(data, "participantAddress", address));
          setSuburb(getSavedValue(data, "participantSuburb", suburb));
          setState(getSavedValue(data, "participantState", state));
          setPostcode(getSavedValue(data, "participantPostcode", postcode));
          setNdisNumberValue(getSavedValue(data, "ndisNumber", ndisNumberValue));
          setNdisStartDate(getSavedValue(data, "ndisPlanStartDate", ndisStartDate));
          setNdisEndDate(getSavedValue(data, "ndisPlanEndDate", ndisEndDate));
          setStartDate(getSavedValue(data, "agreementStartDate", startDate));
          setEndDate(getSavedValue(data, "agreementEndDate", endDate));
        ...
        setSyncStatus("ready");
        setSyncMessage("Editor changes are saved automatically");
      } catch (error) {
        console.error("Kalura editor Service Agreement load error:", error);
        hasLoadedAgreement.current = true;
  ...
  useEffect(() => {
    if (!quoteId || isQuoteBlueprint || isInPreviewer || !isInEditor || !hasLoadedAgreement.current) {
      return;
    }
    ...
    const editorData = {
      heading: heading || "SERVICE AGREEMENT",
    ...
    saveTimer.current = setTimeout(async () => {
      try {
        ...
        console.error("Kalura editor Service Agreement save error:", error);
        setSyncStatus("failed");
    ...
    quoteId, isQuoteBlueprint, isInPreviewer, isInEditor,
    heading, fullName, dateOfBirth, address, suburb, state, postcode,
    ...
    claimNonFace, claimTravelLabour, claimTravelNonLabour, claimCancellation,
    claimReports, conditions, conditionReasons, fundingComponent, fundingDates, fundingNotes,
  ...
  // Participant mode reads and writes Section 24 only. Sections 1–23 and 25
  // come from HubSpot module/quote data and are never sent to this endpoint.
  useEffect(() => {
    if (!quoteId || isQuoteBlueprint || isInPreviewer || isInEditor) {
      return;
    }
    ...
    async function loadParticipantAgreement() {
      try {
        const response = await fetch(`${API_BASE}/${quoteId}/participant`, {
          cache: "no-store",
        });
        ...
        hasLoadedAgreement.current = true;
        setSyncStatus("ready");
        setSyncMessage("Ready for signing");
      } catch (error) {
        console.error("Kalura participant Service Agreement load error:", error);
        hasLoadedAgreement.current = false;
        setSyncStatus("failed");
        setSyncMessage("Unable to load the participant consent form.");
      }
    }

    loadParticipantAgreement();
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(async () => {
      try {
        setSyncStatus("saving");
        setSyncMessage("Saving your consent...");
        ...
        setSyncMessage("Your consent was saved. Updating the signing document...");

        const pollStatus = async () => {
          try {
            const statusResponse = await fetch(
              `${API_BASE}/${quoteId}/republish-status`,
              { cache: "no-store" }
            );
            ...
            if (!statusResponse.ok) {
              throw new Error(
                statusResult?.message || `HTTP ${statusResponse.status}`
              );
            }
            ...
            if (status === "failed") {
              ...
              setSyncMessage(
                message ||
                  "The signing document could not be updated. Please contact KALURA."
              );
              return;
            }
            ...
            console.error(
              "Kalura Service Agreement republish status error:",
              pollError
            );
            setSyncStatus("failed");
            setSyncMessage(
              "We could not confirm the signing document update. Please wait and try again."
            );
          }
        };
        ...
        console.error("Kalura participant Service Agreement save error:", error);
        setSyncStatus("failed");
        setSyncMessage("Unable to save your consent. Please try again.");
      }
    }, 800);
  ...
  return (
    <div className={styles.document}>
      ...
      <h1 className={styles.mainTitle}>{heading || "SERVICE AGREEMENT"}</h1>
      ...
      {isInEditor && (
        ...
        >
          <strong>Editor mode:</strong> You can edit Sections 1–23 and 25 directly in
          the rendered agreement. Changes are saved to the Kalura Service Agreement
          record for this quote. Section 24 is participant-controlled.
        </div>
      )}
      ...
      <fieldset
        ...
        aria-label="Editor-only Agreement Signatures section"
      >
        <div className={styles.majorSection}>
